function Binned_y = Model_3State( parameter_guess, JumpProb, data_to_fit)
%MODEL_3STATE Summary of this function goes here
...
for iterator=1:size(JumpProb,1)
    ...
    y(iterator,:) = Z_corr1(1,iterator).*F_FREE1.*(r./(2*(D_FREE1*curr_dT+LocError^2))).*exp(-r.^2./(4*(D_FREE1*curr_dT+LocError^2))) ...
                    + Z_corr2(1,iterator).*(1-F_BOUND-F_FREE1).*(r./(2*(D_FREE2*curr_dT+LocError^2))).*exp(-r.^2./(4*(D_FREE2*curr_dT+LocError^2))) ...
                    + F_BOUND.*(r./(2*(D_BOUND*curr_dT+LocError^2))).*exp(-r.^2./(4*(D_BOUND*curr_dT+LocError^2))) ;
end
