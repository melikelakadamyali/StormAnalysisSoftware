...
if path~=0
    ...
    if isempty(input_values)==1
        ...
        space_units = input_values{1};
        pixel_size = str2double(input_values{2});
        time_units = input_values{3};
        frame_interval = str2double(input_values{4});        
        if iscell(files)
            for i=1:size(files,2)
                [tracks, ~] = importTrackMateTracks(fullfile(strcat(path,'\'),files{i}),'clipZ',1);
                for k = 1:length(tracks)
                    if size(tracks{k},1)<3 || size(unique(tracks{k}(:,2:3),'rows'),1)<3
                        tracks{k} = [];
                    end
                end
                tracks = tracks(~cellfun('isempty',tracks));
                data_load{i}.tracks = tracks;
                data_load{i}.tracks = tracks_convert_space_time_units(data_load{i}.tracks,space_units,time_units,pixel_size,frame_interval);
                data_load{i}.name = files{i}(1:end-4);
                data_load{i}.type = 'spt'; 
                clear tracks
            end
        else
            [tracks, ~] = importTrackMateTracks(fullfile(strcat(path,'\'),files),'clipZ',1);
            for k = 1:length(tracks)
                if size(tracks{k},1)<3 || size(unique(tracks{k}(:,2:3),'rows'),1)<3
                    tracks{k} = [];
                end
            ...
            tracks = tracks(~cellfun('isempty',tracks));
            data_load{1}.tracks = tracks;
            data_load{1}.tracks = tracks_convert_space_time_units(data_load{1}.tracks,space_units,time_units,pixel_size,frame_interval);
...
% Jean-Yves Tinevez <jeanyves.tinevez@gmail.com> - 2013
    ...
    for i = 1 : nTracks 
        ...
        tracks{i} = A;  
        clear A        
        f = waitbar(i/nTracks,f,'loading track(s)');        
    end
    close(f)
