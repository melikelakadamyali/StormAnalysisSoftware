function image_gaussian_filter(data)
figure()
set(gcf,'name','Gaussian Filter','NumberTitle','off','color','w','units','normalized','position',[0.25 0.15 0.4 0.6],'menubar','none','toolbar','figure')
...
size_slider = uicontrol('style','slider','units','normalized','position',[0.95,0,0.05,1],'value',1,'min',1,'max',5,'sliderstep',[1/1000,1/1000],'Callback',{@size_sld_callback});
...
apply_filter(data{slider_one_value}.image{slider_two_value},size_slider_value)
    ...
    function image_inside(image)
        ...
        colormap(gray)
        title(['size = ',num2str(size_slider_value)])
    end
    ...
    function apply_to_all(~,~)        
        for i = 1:length(data)
            ...
            for j = 1:length(data{i}.image)
                ...
                data{i}.image{j} = imgaussfilt(data{i}.image{j},size_slider_value);
            end
            close(f)
        end
        image_plot(data)
